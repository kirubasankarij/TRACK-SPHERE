...
const Home = () => {
    ...
    return (
        <div className="space-y-12 md:space-y-20 pb-16 md:pb-20">
            {/* Hero Section */}
            <section className="relative min-h-[85vh] flex flex-col items-center justify-center text-center px-4 md:px-8 overflow-hidden rounded-3xl mt-4 md:mt-8">
                {/* Futuristic Background Image */}
                <div className="absolute inset-0 z-0 bg-[url('/background.png')] bg-cover bg-center bg-no-repeat opacity-60 mix-blend-screen" />
                
                {/* Modern Gradient Overlay */}
                <div className="absolute inset-0 z-10 bg-gradient-to-br from-ai-bg/95 via-ai-bg/80 to-primary/20" />
                
                {/* Animated Floating Shapes for extra premium feel */}
                <div className="absolute top-20 left-10 w-32 h-32 bg-blue-400/10 rounded-full blur-3xl animate-pulse" />
                <div className="absolute bottom-20 right-10 w-48 h-48 bg-indigo-400/10 rounded-full blur-3xl animate-pulse delay-700" />
                {/* Content Container */}
                <div className="relative z-20 flex-1 space-y-6 md:space-y-10 max-w-4xl mx-auto px-4 py-12 md:py-20">
                    <div className="inline-block px-4 py-1.5 bg-primary text-white rounded-full text-xs md:text-sm font-bold tracking-wide uppercase shadow-lg shadow-primary/30 animate-bounce">
                        AI-Powered Logistics Flow
                    </div>
                    {/* Hiding duplicate text since it's in the background, or styling it to complement */}
                    <h1 className="text-4xl md:text-6xl lg:text-8xl font-bold font-poppins leading-tight tracking-tighter text-white drop-shadow-2xl">
                        Track<span className="text-primary">Sphere</span> <br />
                        <span className="text-2xl md:text-4xl lg:text-5xl font-orbitron font-medium tracking-widest text-secondary drop-shadow-lg">for the next generation</span>
                    </h1>
                    
                    <p className="text-lg md:text-2xl font-normal font-roboto text-gray-300 max-w-2xl mx-auto leading-relaxed">
                        Optimize your shipment flow with AI-based delay prediction and real-time visualization.
                    </p>

                    <form onSubmit={handleTrack} className="flex flex-col sm:flex-row p-1.5 md:p-2 glass max-w-lg rounded-2xl mx-auto lg:mx-0 gap-2 border-white/10 bg-white/5">
                        <input
                            type="text"
            ...
            <section id="portals" className="pt-8 md:pt-10">
                <div className="text-center mb-10 md:mb-12">
                    <h2 className="text-3xl md:text-5xl font-black mb-4 tracking-tight text-white">Direct User <span className="text-primary">Portals</span></h2>
                    <p className="text-gray-400 font-medium max-w-2xl mx-auto px-4">Access your dedicated interface to manage shipments, track deliveries, or operate the logistics command center.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    {[
                        ...
                        <div key={idx} onClick={portal.navigate} className="glass-card flex flex-col items-center text-center hover:scale-[1.03] transition-all cursor-pointer bg-ai-card/40 border-white/10 hover:border-primary/50 shadow-sm hover:shadow-xl">
                            <div className="h-16 w-16 bg-white/5 text-primary rounded-2xl flex items-center justify-center mb-6 shadow-inner">
                                <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">{portal.icon}</svg>
                            </div>
                            <h3 className="text-2xl font-black mb-2 text-white">{portal.title}</h3>
                            <p className="text-gray-400 text-sm font-medium mb-8 leading-relaxed px-2">{portal.desc}</p>
                            <button className="w-full btn-primary bg-primary hover:bg-primary-dark text-white font-bold py-3.5 rounded-xl transition-all mt-auto shadow-lg shadow-primary/20">
                                {portal.btn}
                            </button>
                        </div>
                    ))}
